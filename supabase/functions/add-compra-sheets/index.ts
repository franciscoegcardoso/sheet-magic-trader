...
serve(async (req) => {
  ...
  try {
    const { insumo, quantidade, unidade, dataCompra, valorCompra } = await req.json();
    
    console.log('Received purchase data:', { insumo, quantidade, unidade, dataCompra, valorCompra });

    if (!insumo || !dataCompra || !valorCompra) {
      return new Response(
    ...
    const sheetName = 'Registro Compras';
    ...
    const formattedDate = dataCompra.split('-').reverse().join('/');

    // Append data: Insumo | Quantidade | Unidade | Data | Valor
    const updateRange = `'${sheetName}'!A${firstEmptyRow}:E${firstEmptyRow}`;
    const updateResponse = await fetch(
      `https://sheets.googleapis.com/v4/spreadsheets/${spreadsheetId}/values/${encodeURIComponent(updateRange)}?valueInputOption=USER_ENTERED`,
      {
        ...
        body: JSON.stringify({
          values: [[insumo, quantidade, unidade, formattedDate, valorCompra]],
        }),
      }
    );
