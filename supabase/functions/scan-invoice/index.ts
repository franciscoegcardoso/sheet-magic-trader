...
serve(async (req) => {
  ...
  try {
    ...
    if (!imageBase64) {
      return new Response(
        JSON.stringify({ error: "Imagem da nota fiscal é obrigatória" }),
    ...
    const response = await fetch("https://ai.gateway.lovable.dev/v1/chat/completions", {
      ...
      body: JSON.stringify({
        ...
        messages: [
          {
            role: "system",
            content: `Você é um assistente especializado em extrair informações de notas fiscais brasileiras.
Analise a imagem da nota fiscal e extraia todos os itens de compra.
Para cada item, extraia:
- nome: nome do produto
- quantidade: quantidade comprada (número)
...
{
  "items": [
    { "nome": "string", "quantidade": number, "valorUnitario": number, "valorTotal": number }
  ],
  "total": number
}

Se não conseguir identificar algum valor, use 0.
Não inclua texto adicional, apenas o JSON.`
    ...
    try {
      ...
      return new Response(
        JSON.stringify({ error: "Erro ao interpretar dados da nota fiscal" }),
        { status: 500, headers: { ...corsHeaders, "Content-Type": "application/json" } }
      );
    }
